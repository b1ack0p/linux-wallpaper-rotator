...
2. Make the scripts executable (only needed the first time):
   ```bash
   chmod +x setup.sh uninstall.sh wallpaper-rotator.sh
   ```
...
- `wallpaper-rotator.sh` — picks the next image (shuffle/sequential), sets it per the chosen desktop
- `wallpaper-rotator.service` — oneshot unit that runs the script (used by the timer)
- `wallpaper-rotator-login.service` — oneshot unit that sets a fresh wallpaper
